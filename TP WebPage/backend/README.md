...
- **`POST /api/auth/forgot-password`** intentionally matches the existing mock contract (email + new password, no verification step) so the frontend integration path stays unchanged. This is not secure for real deployment — replace with an emailed, token-verified reset link before going live. `POST /api/auth/change-password` (current-password-verified, for an already-logged-in user) is the secure path and is what Account Settings should eventually call instead.
