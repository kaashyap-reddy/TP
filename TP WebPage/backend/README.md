...
- **Audit log coverage**: user create/delete/role-change, batch create/update/delete, assignment create/update/delete, submission grading, session create/update/delete, resource upload/verify/delete, feedback submission, and announcement create/update/delete all write to `audit_log` (see `src/services/audit.ts` and each controller). Announcements gained real routes 2026-07-16 (`/api/announcements` — list is role/batch-scoped with per-user read state; the frontend still renders its mock store until it's wired up against a live database). Discussions remain UI-removed with no backend routes, by design.
