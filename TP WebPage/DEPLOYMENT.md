...
- ~~The `forgot-password` endpoint is unauthenticated-by-design~~ — **fixed 2026-07-16**: `POST /auth/forgot-password` now only takes an email and issues a single-use, 1-hour, hashed reset token delivered via the EmailProvider (console-logged until a real provider is connected; exposed in the response only when `EXPOSE_AUTH_TOKENS=true`); `POST /auth/reset-password` verifies it. Account Settings now calls `change-password` (current-password-verified).
